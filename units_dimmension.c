...
UNITS_MAGINITUDE_TYPE get_base_unit_multiplier(const units_si_dimmension* dimmension)
{
    UNITS_MAGINITUDE_TYPE denominator_multiplier = prefix_to_magnitude(units_si_base_unit_to_dimmension(dimmension->type)->prefix);
    UNITS_MAGINITUDE_TYPE numerator_multiplier = prefix_to_magnitude(dimmension->prefix);
    return UNITS_MAGINITUDE_TYPE_DIVIDE(numerator_multiplier, denominator_multiplier);
}
void find_and_increment_then_multiple(struct list* list, const units_si_dimmension* dimmension, const _Bool negative)
{
    for (size_t i = 0; i < list->size; i++)
    {
        ...
        {
            list->multiplier = UNITS_MAGINITUDE_TYPE_MULTIPLY(get_base_unit_multiplier(dimmension), get_base_unit_multiplier(&dimmension2));
...
const struct UNITS_SI_Dimmension* denominator_PRESSURE[] = {&METER, &METER};
// e.g. (M L^-2 T^-2), Please define according to your need
const struct UNITS_SI_Dimmension* numerator_PRESSURE[] = {&NEWTON};
// e.g. (M L^-2 T^-2), Please define according to your need
...
// e.g. (M L^2 T^-2), Please define according to your need
const struct UNITS_SI_Dimmension* numerator_ENERGY[] = {&NEWTON, &METER};
// e.g. (M L^2 T^-2), Please define according to your need
...
const struct UNITS_SI_Dimmension* numerator_POWER[] = {&JOULE};
const struct UNITS_SI_Dimmension* denominator_POWER[] = {&SECOND};
...
const struct UNITS_SI_Dimmension* numerator_ELECTRIC_CHARGE[] = {&AMPERE, &SECOND};
...
const struct UNITS_SI_Dimmension* numerator_ELECTRIC_POTENTIAL[] = {&WATT};
const struct UNITS_SI_Dimmension* denominator_ELECTRIC_POTENTIAL[] = {&AMPERE};
...
const struct UNITS_SI_Dimmension* numerator_ELECTRICAL_RESISTANCE[] = {&VOLT};
const struct UNITS_SI_Dimmension* denominator_ELECTRICAL_RESISTANCE[] = {&AMPERE};
